...
Enable **Cinematic focus — soften the background** for an optional subject-focused look. It includes lighting normalization, finds regions where image detail changes over the selected sequence, and keeps those regions sharp while gently blurring and dimming quieter areas. A single feathered focus area covers the entire sequence, so the focus does not jump between captures. It works best with a fixed camera and a mostly stationary background; moving shadows or camera movement can also influence the detected region. If no changing detail is detected, or the photos have incompatible aspect ratios, the focus effect is skipped and lighting correction still applies. It does not add frames or change video duration: choose **Motion interpolation** separately for smoother movement, or **Repeat photos** to hold the captured stages longer. The timing rings stay sharp because they are drawn after the effect. Cinematic focus is off by default, remembered per project, and saved with each export.
