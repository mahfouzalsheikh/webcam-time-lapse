...
3. Optionally check **Normalize brightness and white balance (reduce flicker)** before exporting. It is off by default and applies only to the MP4; original photos and editor previews stay unchanged. It matches each selected frame's brightness and color balance to the sequence's median channel levels, with limited correction to avoid excessive amplification of dark frames. It works best with a fixed view and may also reduce intentional lighting or color changes; clipped highlights and deep shadows cannot be recovered. Normalized exports take longer and use temporary disk space for corrected frames, which is released after completion or failure. The option is recorded with each export and retained across restarts.
4. Click **Export final video** and download the MP4 when ready. The export saves the included frame IDs, resolution, playback speed, and lighting normalization choice at the moment it is queued. Later frame edits or new captures do not alter that export, even if rendering restarts.
